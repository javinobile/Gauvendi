...
export class Helper {
  ...
  static getDateListFromDateRange(startDate: string, endDate: string): string[] {
    ...
    const dates = Array.from({ length: totalDays }, (_, i) =>
      format(addDays(start, i), DATE_FORMAT)
    );
  ...
  static checkValidFields(body: any, validFields: string[]) {
    const invalidFields = body.flatMap((item) =>
      Object.keys(item).filter((key) => !validFields.includes(key))
    );
  ...
  /**
   * Calculate total cityTaxAmount from cityTaxCharges array
   * Similar to Java's getCityTaxPerReservation method - aggregates grossAmount from all cityTax charges
   */
  static calculateCityTaxAmount(
    cityTaxCharges?: Array<{ amount?: { grossAmount?: number } }>
  ): number {
    if (!cityTaxCharges || cityTaxCharges.length === 0) {
      return 0;
    }

    return cityTaxCharges.reduce((total, charge) => {
      const grossAmount = charge.amount?.grossAmount || 0;
      return total + grossAmount;
    }, 0);
  }

  static convertNewLineForHtml(text: string): string {
...
export function splitDateRangeIntoChunks(fromDate: Date, toDate: Date, chunkSize = 7) {
