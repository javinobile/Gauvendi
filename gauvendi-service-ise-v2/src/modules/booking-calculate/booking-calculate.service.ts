...
import { surchargeChargeServiceCodes } from '../hotel-amenity/services/hotel-amenity.service';
...
import { RatePlanRepository } from '../hotel-rate-plan/repositories/rate-plan.repository';
import { HotelRetailCategoryRepository } from '../hotel-retail-category/repositories/hotel-retail-category.repository';
import { HotelTaxSettingRepository } from '../hotel-tax-settings/hotel-tax-setting.repository';
import { HotelTaxRepository } from '../hotel-tax/hotel-tax.repository';
import { HotelRepository } from '../hotel-v2/repositories/hotel.repository';
import { HotelService } from '../hotel-v2/services/hotel.service';
import { RatePlanExtraServicesRepository } from '../rate-plan-extra-services/rate-plan-extra-services.repository';
import { RoomProductExtraRepository } from '../room-product-extra/room-product-extra.repository';
import { RoomProductRetailFeatureRepository } from '../room-product-retail-feature/repositories/room-product-retail-feature.repository';
import { RoomProductStandardFeatureRepository } from '../room-product-standard-feature/repositories/room-product-retail-feature.repository';
import { RoomProductRepository } from '../room-product/repositories/room-product.repository';
import { TranslationRepository } from '../translation/repositories/translation.repository';
import { BookingTaxService } from './booking-tax.service';
import { AvailableRoomCapacityFilterDto } from './dtos/available-room-capacity-filter.dto';
import { BookingPricingDto } from './dtos/booking-pricing.dto';
import {
  CalculateBookingPricingInputDto,
  CalculateReservationPricingInputDto
} from './dtos/calculate-booking-pricing-input.dto';
import { HotelPricingDecimalRoundingRuleDto } from './dtos/hotel-pricing-decimal-rounding-rule.dto';
import { ReservationPricingDto, TaxDto } from './dtos/reservation-pricing.dto';
...
export class BookingCalculateService {
  ...
  async calculateBookingPricing(input: CalculateBookingPricingInputDto) {
    // return this.hotelConfigurationRepository.getHotelConfigurationByTypes(input);
    ...
    try {
      ...
      let [
        ...
        this.getTranslatedHotelTax(hotel.id, input.translateTo),
      ...
      const hotelTaxMapByCode = groupByToMapSingle(hotelTaxes, (item) => item.code);
      const hotelTaxMapById = groupByToMapSingle(hotelTaxes, (item) => item.id);
      ...
      for (const reservation of input.reservations) {
        ...
        const taxSettings = await this.hotelService.getHotelTaxSettings(hotel.id, [
          ...serviceCodes,
          ...surchargeChargeServiceCodes
        ]);
        ...
        const toDate = format(
          subDays(Helper.parseDateToUTC(reservation.departure), 1),
          DATE_FORMAT
        );
        ...
        const { roomProductRatePlanResults, amenityPricingList } =
          await this.roomProductPricingCalculateService.calculateRoomProductPricing({
            ...
            isCalculateCityTax:
              input.isCityTaxIncluded === false
                ? pricingConfiguration.cityTaxDisplay === 'INCLUDED'
                : true,
        ...
        const mostBeneficialCxlPolicies = await this.paymentService.getMostBeneficialCxlPolicy({
          ratePlanIds: [ratePlan.id],
          hotelId: hotel.id,
          fromDate: fromDate,
          toDate: toDate,
          translateTo: input.translateTo,
          defaultCxlPolicyCodes: ratePlans.map((ratePlan) => ({
        ...
        if (mostBeneficialCxlPolicy?.hotelCancellationPolicy) {
          const translation = mostBeneficialCxlPolicy.hotelCancellationPolicy.translations.find(
            (translation) => translation.languageCode === input.translateTo
          );

          if (translation) {
            if (translation.name) {
              mostBeneficialCxlPolicy.hotelCancellationPolicy.name = translation.name;
            }
            if (translation.description) {
              mostBeneficialCxlPolicy.hotelCancellationPolicy.description = translation.description;
            }
          }
        ...
        const mostBeneficialPaymentTerms = await this.paymentService.getMostBeneficialPaymentTerm({
          ratePlanIds: [ratePlan.id],
          hotelId: hotel.id,
          fromDate: new Date(reservation.arrival),
          toDate: new Date(reservation.departure)
        });
        ...
        if (mostBeneficialPaymentTerm?.hotelPaymentTerm) {
          let translation;
          if (input.translateTo) {
            translation = mostBeneficialPaymentTerm.hotelPaymentTerm.translations.find(
              (translation) =>
                `${translation.languageCode}`.toLowerCase() === `${input.translateTo}`.toLowerCase()
            );
          }
          ...
          reservationPricing.hotelPaymentTerm = {
            ...
            name: translation?.name || mostBeneficialPaymentTerm.hotelPaymentTerm.name,
        ...
        reservationPricing = this.mergeTaxDetailsMap(reservationPricing, hotelTaxMapByCode);
      ...
      const bookingPricing = this.buildBookingPricing(
        ...
        hotelTaxMapByCode, // hotelTaxMap - not available in current context
        hotelTaxMapById,
  ...
  buildBookingPricing(
    ...
    hotelId: string,
    currencyCode: string,
    hotelTaxMapByCode?: Map<string, HotelTax>,
    hotelTaxMapById?: Map<string, HotelTax>,
    ...
    try {
          ...
          pricing.accommodationTaxList.forEach((tax) => {
            ...
            if (accommodationTaxMap.has(taxCode)) {
              ...
              accommodationTaxMap.set(taxCode, {
                amount: tax.amount || 0,
                taxDto: hotelTaxMapByCode?.get(taxCode) || tax
              });
            }
          });
        }
      ...
      reservationPricingList.forEach((pricing) => {
        if (pricing.taxDetailsMap) {
          const taxSettingList = pricing.amenityPricingList.flatMap((item) => item.taxSettingList);

          for (const [taxCode, amount] of Object.entries(pricing.taxDetailsMap)) {
            const hotelTax = hotelTaxMapByCode?.get(taxCode);
            if (hotelTax) {
              const taxSetting = taxSettingList.find((item) => item.taxCode === taxCode);
              bookingTaxList.push({
                ...hotelTax,
                name: taxSetting ? `${hotelTax.name} - ${taxSetting.serviceCode}` : hotelTax.name,
                code: taxCode,
                amount
              });
            }
          ...
          Object.entries(pricing.taxDetailsMap).forEach(([taxCode, amount]) => {
            const currentAmount = generalTaxMap.get(taxCode) || 0;
            generalTaxMap.set(taxCode, currentAmount + (Number(amount) || 0));
          });
        }

        if (pricing.taxAccommodationDetailsMap) {
          for (const [taxCode, amount] of Object.entries(pricing.taxAccommodationDetailsMap)) {
            const hotelTax = hotelTaxMapByCode?.get(taxCode);
            if (hotelTax) {
              bookingTaxList.push({
                ...hotelTax,
                name: accommodationTranslation
                  ? `${hotelTax.name} - ${accommodationTranslation}`
                  : hotelTax.name,
                code: taxCode,
                amount
              });
          ...
          Object.entries(pricing.taxAccommodationDetailsMap).forEach(([taxCode, amount]) => {
            const currentAmount = generalTaxMap.get(taxCode) || 0;
            generalTaxMap.set(taxCode, currentAmount + (Number(amount) || 0));
          });
        }
      ...
      bookingTaxList = Array.from(generalTaxMap.entries()).map(([taxCode, amount]) => ({
        ...(hotelTaxMapByCode?.get(taxCode) || {}),
        code: taxCode,
        amount
      }));
   ...
   */
  private async getTranslatedHotelTax(hotelId: string, translateTo?: string): Promise<HotelTax[]> {
    try {
      const hotelTaxList = await this.hotelTaxRepository.findAll({
        hotelId
      });
