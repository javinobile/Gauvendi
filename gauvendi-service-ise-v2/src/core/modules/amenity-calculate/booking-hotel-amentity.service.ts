...
import { RatePlanDerivedSettingRepository } from 'src/modules/hotel-rate-plan/repositories/rate-plan-derived-setting.repository';
import { RatePlanExtraServicesRepository } from 'src/modules/rate-plan-extra-services/rate-plan-extra-services.repository';
import { RoomProductExtraRepository } from 'src/modules/room-product-extra/room-product-extra.repository';
...
@Injectable()
export class BookingHotelAmenityService {
  constructor(
  ...
  async getBookingRatePlanAmenities(filter: {
    ...
    const allRatePlanIds = [...ratePlanIds, ...masterRatePlanIds];
  ...
  private getRatePlanAmenityDailyOrDefault(filter: {
    ratePlanId: string;
    dates: string[];
