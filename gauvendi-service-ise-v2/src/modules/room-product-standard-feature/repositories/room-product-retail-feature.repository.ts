  ...
  async getRoomProductStandardFeatures(filter: RoomProductStandardFeatureFilterDto) {
    try {
      ...
      const mappedResultPromises = result.map(async (item) => {
        const foundTranslation = item.standardFeature.translations?.find(
          (translation) =>
        ...
        return {
          ...
          standardFeature: {
            ...
            name: foundTranslation?.name || item.standardFeature?.name,
            description: foundTranslation?.description || item.standardFeature?.description,
