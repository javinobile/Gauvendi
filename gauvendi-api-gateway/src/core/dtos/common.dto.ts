...
export class ResponseContent<T> {
  ...
  static success<T>(data: T, message?: string): ResponseContent<T> {
    return new ResponseContent(ValidationMessage.success(message || "Operation successful"), ResponseContentStatusEnum.SUCCESS, data);
  }

  static error<T>(message: string, data?: T): ResponseContent<T> {
    return new ResponseContent(ValidationMessage.error(message), ResponseContentStatusEnum.ERROR, data);
  }
}
