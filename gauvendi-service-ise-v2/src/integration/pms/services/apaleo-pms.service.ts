...
import { CountryRepository } from 'src/modules/core/repositories/country.repository';
...
} from '../constants/apaleo-pms.const';
import {
...
export class ApaleoPmsService extends AbstractPmsService {
  ...
  constructor(
    ...
    this.distributionBaseUrl =
      this.configService.get(ENVIRONMENT.PMS_APALEO_DISTRIBUTION_API_URL) ?? '';
  ...
  async getAccessToken(refreshToken: string, propertyId: string) {
    ...
    try {
      const clientId = this.configService.get(ENVIRONMENT.PMS_APALEO_IDENTITY_CLIENT_ID) ?? '';
      const clientSecret =
        this.configService.get(ENVIRONMENT.PMS_APALEO_IDENTITY_CLIENT_SECRET) ?? '';
  ...
  async createReservationForApaleo(input: ReservationsCreatePmsInput) {
    const { booking, connector, reservationTimeSlices, hotel } = input;
    ...
    for (const [index, reservation] of reservations.entries()) {
      ...
      for (const timeSlice of reservationTimeSlices) {
        ...
        assignUnitsInput.push({
          ...
          from: convertToHotelTimezone(timeSlice.fromTime, hotel.timeZone ?? ''),
          to: convertToHotelTimezone(timeSlice.toTime, hotel.timeZone ?? ''),
  ...
  private async assignUnits(assignUnitsInput: any[], hotelId: string, connector: Connector | null) {
    const baseUrl = `${this.baseUrl}${PMS_APALEO_API_URI.ASSIGN_UNITS}`;
    const accessToken = await this.getAccessToken(connector?.refreshToken ?? '', hotelId ?? '');
    if (!accessToken) {
      this.logger.warn(`No access token found for hotel ${hotelId} and connector ${connector?.id}`);
      return;
    }
    ...
    await processInBatches(
      ...
      async (assignUnit: any) => {
        try {
          const url = baseUrl.replace('{id}', assignUnit.id).replace('{unitId}', assignUnit.unitId);
          const body = assignUnit;
          const headers = this.buildHeaders(accessToken);
          const cmd = getCurlCommand(url, 'PUT', headers, body);
          this.logger.log(`Curl command assign units apaleo: ${JSON.stringify(cmd)}`);
          const response = await firstValueFrom(this.httpService.put(url, body, { headers }));
          const data = response.data as ApaleoCreateBookingResponseDto;
    ...
    await processInBatches(
      ...
      async (reservation: Reservation) => {
        ...
        if (reservation.isLocked) {
          return await this.unlockUnits(accessToken, reservation.mappingReservationCode);
        } else {
          return await this.lockUnits(accessToken, reservation.mappingReservationCode);
        }
      }
    );
  }
  ...
  async lockUnits(accessToken: string, reservationMappingCode: string) {
    try {
      const lockUnitsUrl = `${PMS_APALEO_API_URI.LOCK_UNIT}`.replace(
        '{id}',
        reservationMappingCode
      );
      const headers = this.buildHeaders(accessToken);
      return await firstValueFrom(this.httpService.put(lockUnitsUrl, {}, { headers }));
    } catch (error) {
      const err = error.response?.data;
      this.logger.error(`Error locking units: ${JSON.stringify(err)}`);
      return null;
    }
  }
  ...
  async unlockUnits(accessToken: string, reservationMappingCode: string) {
    try {
      const unlockUnitsUrl = `${PMS_APALEO_API_URI.UNLOCK_UNIT}`.replace(
        '{id}',
        reservationMappingCode
      );
      const headers = this.buildHeaders(accessToken);
      return await firstValueFrom(this.httpService.put(unlockUnitsUrl, {}, { headers }));
    } catch (error) {
      const err = error.response?.data;
      this.logger.error(`Error unlocking units: ${JSON.stringify(err)}`);
      return null;
    }
  }
  ...
  async getFolios(
    ...
    try {
      ...
      for (const reservationMappingCode of reservationMappingCodes) {
        queryParams.append('reservationIds', reservationMappingCode);
      }
      const headers = this.buildHeaders(accessToken);
      const response = await firstValueFrom(
  ...
  async buildCreateBookingInput(
    ...
    const { booker, booking, guest } = input;
    ...
    const guestCountryCode = guest.countryId ? await this.getCountryCode(guest.countryId) : '';
    ...
    const reservations = await this.buildReservations(input, guestCountryCode, currencyCode);
  ...
  private buildPaymentAccount(
    ...
    const paymentMode: any = booking.reservations[0].hotelPaymentModeCode;
    const outsideCases = [
      HotelPaymentModeCodeEnum.GUAWDE,
      HotelPaymentModeCodeEnum.PAYPAL,
      HotelPaymentModeCodeEnum.PMDOTH
    ];
    if (outsideCases.includes(paymentMode) || !bookingTransaction?.accountNumber) {
      return null;
    }
  ...
  private buildCreditCard(
    ...
    const { booker, bookingInput, booking } = input;
    const paymentMode: any = booking.reservations[0].hotelPaymentModeCode;
    const outsideCases = [
      HotelPaymentModeCodeEnum.GUAWDE,
      HotelPaymentModeCodeEnum.PAYPAL,
    ...
    return {
      ...
      cvc: bookingInput.creditCardInformation?.cvv ?? '',
  ...
  private async buildReservations(
    ...
    const { booking, bookingInput, booker, guest, hotel, reservationTimeSlices } = input;
    const reservations = booking.reservations;
    ...
    const reservationsMapped: ApaleoCreateReservationDto[] = await Promise.all(
      reservations.map(async (reservation) => {
        ...
        const companyId = await this.getApaleoCompanyId(
          reservation.companyId,
          hotel,
          input.connector
        );
        ...
        const reservationMap: ApaleoCreateReservationDto = {
          arrival: convertToHotelTimezoneDate(reservation.arrival, hotel.timeZone ?? ''),
          departure: convertToHotelTimezoneDate(reservation.departure, hotel.timeZone ?? ''),
          adults: reservation.adults ?? 0,
          childrenAges: reservation.childrenAges ?? [],
          // comment: reservation.note || '',
          externalId: reservation.reservationNumber || '',
          guestComment: reservation.note || 'No comment',
          ...
          guaranteeType:
            ApaleoGuaranteeTypeMapping[bookingInput.bookingInformation.hotelPaymentModeCode] ||
            ApaleoGuaranteeType.CREDIT_CARD,
  ...
  private async buildReservationServices(
    ...
    try {
      pmsServices =
        (
          await this.getServices(reservation.accessToken, {
  ...
  async getServices(accessToken: string, params: ApaleoGetServiceListDto) {
    ...
    const url = `${this.configService.get(ENVIRONMENT.PMS_APALEO_API_URL)}${PMS_APALEO_API_URI.GET_SERVICE_LIST}`;
    const queryParams = new URLSearchParams();

    queryParams.append('propertyId', params.propertyId);
    queryParams.append('pageNumber', params.pageNumber?.toString() || '1');
    queryParams.append('pageSize', params.pageSize?.toString() || '6585');
