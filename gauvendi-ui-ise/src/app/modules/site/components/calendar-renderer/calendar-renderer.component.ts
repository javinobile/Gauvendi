...
@Component({
  ...
  templateUrl: './calendar-renderer.component.html',
  styleUrls: ['./calendar-renderer.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CalendarRendererComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Output() recalculateMaxCheckoutDate = new EventEmitter();
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['calendarDailyRates'] && this.isMonthChanged) {
      this.recalculateMaxCheckoutDate.emit();
    }

    if (
      changes.hasOwnProperty('calendarDailyRates') ||
      changes.hasOwnProperty('activeDate') ||
      (changes.hasOwnProperty('selectedValue') && this.calendarDailyRates)
    ) {
    ...
    for (let week = 0; week < this.MAX_ROW; week++) {
      ...
      for (let day = 0; day < this.MAX_COL; day++) {
        ...
        if (
          ...
          tempDate.isDisable = !(
            from &&
            DateUtils.isSameDateOnCalendar(tempDate?.value, this.maxDate)
          );
        }
