...
import {
  AmenityDataProviderService,
  AmenityWithType
} from '@src/core/modules/amenity-calculate/amenity-data-provider.service';
...
import {
  ...
  HotelAmenityService,
  PET_AMENITY_CODE
} from '@src/modules/hotel-amenity/hotel-amentity.service';
...
import { AmenityPricingDto, RoomProductSalesPlanDto } from '../dtos/booking.dto';
import { CalculateRoomProductPricingInputDto } from '../dtos/calculate-room-product-pricing-input.dto';
import { DailySellingRateDto } from '../repositories/daily-selling-rate.dto';
import { RoomProductDailySellingPriceRepository } from '../repositories/room-product-daily-selling-price.repository';
import { BookingAmenityCalculateService } from './booking-amenity-calculate.service';
import { BookingCityTaxCalculateService } from './booking-city-tax-calculate.service';
import { CalculateAllocateCapacityResult } from './calculate-allocation.service';
...
export class RoomProductPricingCalculateService {
  ...
  async calculateRoomProductPricing(input: CalculateRoomProductPricingInputDto) {
    ...
    for (const roomProductInput of input.roomProductInputs || []) {
      ...
      for (const roomProductRatePlan of roomProductRatePlansInRoomProduct) {
        ...
        // const roomProductExtrasInRoomProductRatePlan = roomProductExtras.filter(
        //   (r) => r.roomProductId === roomProduct.id
        // );

        // const ratePlanExtraServicesInRoomProductRatePlan = ratePlanExtraServices.filter(
        //   (r) => r.ratePlanId === roomProductRatePlan.ratePlanId
        // );

        // const mandatoryExtraServices = this.hotelAmenityService.getExtraServicesByType(
        //   hotelAmenities,
        //   ratePlanExtraServicesInRoomProductRatePlan,
        //   roomProductExtrasInRoomProductRatePlan,
        //   [RatePlanExtraServiceType.MANDATORY]
        // );
        // const mandatoryExtraServiceIds = mandatoryExtraServices.map((service) => service.id);
        // const mandatoryExtraServicePrices = hotelAmenityPrices.filter((price) =>
        //   mandatoryExtraServiceIds.includes(price.hotelAmenityId)
        // );

        // const addOnExtraServices = this.hotelAmenityService.getExtraServicesByType(
        //   hotelAmenities,
        //   ratePlanExtraServicesInRoomProductRatePlan,
        //   roomProductExtrasInRoomProductRatePlan,
        ...
        // const addOnExtraServiceIds = addOnExtraServices.map((service) => service.id);
        // const addOnExtraServicePrices = hotelAmenityPrices.filter((price) =>
        //   addOnExtraServiceIds.includes(price.hotelAmenityId)
        // );

        // const includedExtraServices = this.hotelAmenityService.getExtraServicesByType(
        //   hotelAmenities,
        //   ratePlanExtraServicesInRoomProductRatePlan,
        //   roomProductExtrasInRoomProductRatePlan,
        //   [RatePlanExtraServiceType.INCLUDED]
        // );
        // const includedExtraServiceIds = includedExtraServices.map((service) => service.id);
        ...
        dto.totalOccupancySurchargeAmount = totalOccupancySurchargeAmount;
        ...
        if (isCalculateCityTax) {
          const calculatedCityTax = this.cityTaxCalculateService.calculateBookingCityTax({
  ...
  private async calculateExtraAmenityPrices(input: {
    ...
    const { extra, included, mandatory } = this.amenityDataProviderService.combineBookingAmenities({
  ...
  private async calculateExtraOccupancySurcharge(input: CalculateExtraOccupancySurchargeInput) {
    ...
    for (const roomProductRatePlan of roomProductRatePlans) {
      ...
      - Extra bed:
        ...
        results.push({
          taxAmount: roomProductDailySellingPrice?.taxAmount ?? 0,
          ...
          doubleOccupancyRate: doubleOccupancyRate,
