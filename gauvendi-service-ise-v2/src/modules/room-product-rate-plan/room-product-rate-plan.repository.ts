...
import { FindOptionsSelect, In, Raw, Repository } from 'typeorm';
import { RoomProductRatePlanFilterDto } from '../hotel-rate-plan/dtos/room-product-rate-plan-filter.dto';
...
export class RoomProductRatePlanRepository extends BaseService {
  ...
  findAll(
    ...
    if (filter.roomProductIdList && filter.roomProductIdList.length > 0) {
      where.roomProductId = In(filter.roomProductIdList);
    }
    ...
    if (filter.ratePlanStatusList && filter.ratePlanStatusList.length > 0) {
      where.ratePlan = {
        status: In(filter.ratePlanStatusList)
      };
    }
    ...
    if (filter.isAutomatePricing !== undefined && filter.isAutomatePricing !== null) {
      // where.isAutomatePricing = filter.isAutomatePricing;
    }
