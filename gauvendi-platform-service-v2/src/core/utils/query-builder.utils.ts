...
export class QueryBuilderUtils {
  ...
  static setEqualOrInQuery<T extends ObjectLiteral>(
    ...
    const newQueryBuilder = queryBuilder.clone();
    if (Array.isArray(values)) {
      if (values.length === 1) {
        newQueryBuilder.andWhere(`${field} = :${paramName}`, { [paramName]: values[0] });
      } else {
        newQueryBuilder.andWhere(`${field} IN (:...${paramName})`, { [paramName]: values });
      }
    } else {
      newQueryBuilder.andWhere(`${field} = :${paramName}`, { [paramName]: values });
    }
  ...
  static setNotEqualOrNotInQuery<T extends ObjectLiteral>(
    ...
    if (Array.isArray(values)) {
      if (values.length === 1) {
        ...
        newQueryBuilder.andWhere(`${field} NOT IN (:...${paramName})`, { [paramName]: values });
      }
    } else {
      newQueryBuilder.andWhere(`${field} != :${paramName}`, { [paramName]: values });
    }
  ...
  static addLikeConditions<T extends ObjectLiteral>(
    ...
    const newQueryBuilder = queryBuilder.clone();
    newQueryBuilder.andWhere(`(${conditions})`, parameters);
    return newQueryBuilder;
  }
