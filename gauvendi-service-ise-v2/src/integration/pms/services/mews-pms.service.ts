...
import { PMS_MEWS_API_URI } from '../constants/mews-pms.const';
import {
  ProductOrderDto,
...
export class MewsPmsService extends AbstractPmsService {
  ...
  constructor(
    ...
    this.distributionBaseUrl = this.configService.get(ENVIRONMENT.PMS_MEWS_CONNECTOR_API_URL) ?? '';
  }

  async createReservationForMews(input: ReservationsCreateMewsInput) {
    const url = `${this.distributionBaseUrl}${PMS_MEWS_API_URI.CREATE_RESERVATION}`;
    ...
    this.logger.log(`Curl command create reservation mews: ${JSON.stringify(cmd)}`);
    try {
  ...
  async getAllAgeCategoriesForMews(
    ...
    const body = {
      ClientToken: this.configService.get(ENVIRONMENT.PMS_MEWS_CLIENT_TOKEN) ?? '',
    ...
    const cmd = getCurlCommand(url, 'POST', {}, body);
    this.logger.log(`Curl command age categories: ${JSON.stringify(cmd)}`);
    try {
      const response = await firstValueFrom(this.httpService.post(url, body));
  ...
  async buildReservationCreateInput(
    ...
    const { booking, connector, bookingInput } = input;
    ...
    const hotelTax = await this.hotelTaxRepository.getHotelTax({
      hotelId: booking?.hotelId ?? ''
    });
    ...
    for (const reservation of reservations) {
      const roomProductMapping = await this.availabilityService.getRoomProductMappingPms({
        hotelId: booking?.hotelId ?? '',
        roomProductId: reservation.roomProductId ?? '',
        roomUnitIds: roomUnitIds
      });
      roomProductMappingList.set(reservation.id, roomProductMapping);
  ...
  getTimeUnitPrices(input: ReservationsCreateMewsInput, hotelTax: HotelTax | null) {
    const { booking, bookingInput } = input;
    ...
    for (const reservation of reservations) {
      ...
      const timeUnitPrice = Array.from({ length: nights }, (_, index) => ({
        ...
        Amount: {
          ...
          Currency: bookingInput.bookingInformation.bookingPricing?.currencyCode ?? '',
  ...
  async getCustomerByEmail(input: ReservationsCreateMewsInput) {
    ...
    const cmd = getCurlCommand(url, 'POST', {}, body);
    this.logger.log(`Curl command get customer by email: ${JSON.stringify(cmd)}`);
    try {
      const response = await firstValueFrom(this.httpService.post(url, body));
  ...
  async addCustomerForMews(input: ReservationsCreateMewsInput) {
    ...
    const cmd = getCurlCommand(url, 'POST', {}, body);
    this.logger.log(`Curl command add customer: ${JSON.stringify(cmd)}`);
    try {
      const response = await firstValueFrom(this.httpService.post(url, body));
