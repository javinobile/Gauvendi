...
export class BookingValidatorService {
  ...
  private validateRequiredFields(payload: RequestBookingDto) {
    ...
    if (!required?.firstName || !required?.emailAddress) {
  ...
  async validateRoomAndProductAvailabilityProposal(
    ...
    const roomAvailability = await this.availabilityService.roomProductCheckAvailabilityProposal({
      hotelId: hotel.id,
  ...
  async validateRoomAndProductAvailability(
    payload: RequestBookingDto,
    hotel: Hotel
  ): Promise<RoomAvailabilityDto[]> {
    const roomAvailability = await this.availabilityService.roomProductCheckAvailability({
      hotelId: hotel.id,
      requestRooms: payload.bookingInformation.reservationList?.map((r) => ({
        roomProductId: r.roomProductId ?? '',
        arrival: r.arrival,
