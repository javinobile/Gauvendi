...
export class S3Service {
   ...
   */
  async getFileBuffer(s3Url: string): Promise<Buffer | null> {
    try {
      const bucketName = this.configService.get<string>(ENVIRONMENT.S3_BUCKET_NAME) || '';
      ...
      if (s3Url.includes('amazonaws.com/')) {
        ...
        key = urlParts.slice(urlParts.indexOf(bucketName) + 1).join('/');
      } else if (s3Url.startsWith('https://')) {
        // Handle CDN URLs - extract key part
        const url = new URL(s3Url);
        key = url.pathname.substring(1); // Remove leading slash
      }

      this.logger.log(`[getFileBuffer] Downloading file from S3: ${key}`);
      ...
      if (!response.Body) {
        this.logger.error(`[getFileBuffer] No data received for key: ${key}`);
        return null;
      }
      ...
      this.logger.error(`[getFileBuffer] Failed to download file from S3: ${s3Url}`, error);
      return null;
    }
  }
}
