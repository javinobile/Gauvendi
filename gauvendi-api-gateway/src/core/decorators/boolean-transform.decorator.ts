...
export function BooleanTransform(options: BooleanTransformOptions = {}) {
  const { allowNull = false, message, trueValues = ["true", "1", "yes"], falseValues = ["false", "0", "no", ""], debug = false } = options;
 ...
 */
export const RequiredBoolean = (options: Omit<BooleanTransformOptions, "allowNull"> = {}) => BooleanTransform({ ...options, allowNull: false });

/**
 * Shorthand for optional boolean fields
 */
export const OptionalBoolean = (options: Omit<BooleanTransformOptions, "allowNull"> = {}) => BooleanTransform({ ...options, allowNull: true });
